		...
		>
			<div class="flex align-items-center mt-4 flex-column gap-2" style="width: 490px">
				...
				<div class="w-full flex justify-content-center gap-3">
					...
					<img
						:src="`/assets/img/grupos/${store.getUsuario().grupo}.png`"
						width="60px"
						height="60px"
					...
					<div class="flex flex-column gap-4">
						...
						<div class="live flex align-items-center decorador gap-2 p-2">
							...
							<div class="informacion flex flex-column font-gamers uppercase text-white text-left line-height-2">
								...
								<span class="font-bold text-2xl">{{ formatHoras(store.getUsuario().last_live_duration_mes_actual) }}</span>

								<span class="text-sm font-bold">
									En {{ meses[iAnterior] }}:
									<span class="font-bold color-rojo text-2xl">{{ formatHoras(store.getUsuario().last_live_duration_mes_anterior) }}</span>
									<span :class="'ml-2 text-sm font-bold text-2xl ' + porcentaje(store.getUsuario().porcentaje_achieved_2)">
										...
										<img
											:src="
												mayorTiempo(store.getUsuario().last_live_duration_mes_actual, store.getUsuario().last_live_duration_mes_anterior)
											"
											class="svg"
										/>
									</span>
						...
						<div class="dias_habiles_actual flex align-items-center decorador gap-2 p-2">
							...
							<div class="informacion flex flex-column font-gamers uppercase text-white text-left line-height-2">
								...
								<span class="font-bold text-2xl">
									{{ store.getUsuario().dias_validos_mes_actual }}
									{{ parseInt(store.getUsuario().dias_validos_mes_actual) == 1 ? "día" : "días" }}
									<span :class="'ml-2 text-2xl ' + porcentaje(store.getUsuario().porcentaje_achieved_3)">
										{{ store.getUsuario().porcentaje_achieved_3 }}
										<img
											:src="subioBajo(store.getUsuario().dias_validos_mes_actual, store.getUsuario().dias_validos_mes_anterior)"
											class="svg"
										/>
									</span>
								</span>
						...
						<div class="incorporacion flex align-items-center decorador gap-2 p-2">
							...
							<div class="informacion flex flex-column font-gamers uppercase text-white text-left line-height-2">
								...
								<span class="font-bold text-2xl">
									{{
										store.getUsuario().dias_since_joining_agency + (store.getUsuario().dias_since_joining_agency == 1 ? " día" : " días")
									}}
								</span>
							</div>
						</div>
					...
					<div class="flex flex-column gap-4">
						<div class="fans flex align-items-center decorador gap-2 p-2">
							...
							<div class="informacion flex flex-column font-gamers uppercase text-white text-left line-height-2">
								...
								<span class="font-bold text-2xl">{{ parseInt(store.getUsuario().new_fans_mes_actual).toLocaleString() }}</span>
								<span class="text-sm">
									En {{ meses[iAnterior] }}:
									<span
										:class="
											'ml-2 font-bold text-2xl ' +
											(parseInt(store.getUsuario().new_fans_mes_actual) > parseInt(store.getUsuario().new_fans_mes_anterior)
												? 'text-shadow-green'
												: 'text-shadow-red')
										"
									>
										{{ parseInt(store.getUsuario().new_fans_mes_anterior).toLocaleString() }}
									</span>
						...
						<div class="dias_habiles_anterior flex align-items-center decorador gap-2 p-2">
							...
							<div class="informacion flex flex-column font-gamers uppercase text-white text-left line-height-2">
								...
								<span class="font-bold text-2xl">
									{{ store.getUsuario().dias_validos_mes_anterior }}
									{{ parseInt(store.getUsuario().dias_validos_mes_anterior) == 1 ? "día" : "días" }}
								</span>
							</div>
						</div>
...
export default {
	...
	methods: {
		...
		getSegundos(tiempo) {
			...
			const segundos = tiempo.replace(regex, (match, horas, minutos, segundos) => {
				const totalSegundos = parseInt(horas) * 60 * 60 + parseInt(minutos) * 60 + parseInt(segundos);
	...
	created() {
		...
		this.fechaAct = `${(f.getDate() < 10 ? "0" : "") + f.getDate()} ${this.meses[f.getMonth()]} ${this.store
			.getUsuario()
			.fecha_actualizacion.slice(11, 16)}`;
