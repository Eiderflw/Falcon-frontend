	...
	<div class="game">
		...
		<div class="form-container" v-show="form">
			...
			<div class="form">
				...
				<button :disabled="btnDisable" class="sign" type="button" @click="startGame()">Ingresar</button>
			</div>
			...
			<div class="social-icons">
				...
				<a aria-label="Log in with Instagram" class="icon" href="https://www.instagram.com/falconsagencylatam/">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
						<path
		...
		<div class="controles">
			<button v-show="buttonStart" @click="iniciarJuego()" class="buttonGame btnEmpezar">Empezar</button>
			<button v-show="!form" @click="reiniciar()" class="buttonGame">Volver</button>
			<button v-show="!form" @click="togglePause()" :class="pause ? 'btnRojo' : ''" class="buttonGame">
				{{ pause ? "Pausado" : "Pausar" }}
			</button>
		...
		<div class="volumen" v-show="!form">
			<Slider v-model="volumen" orientation="vertical" v-on:slideend="changeVolumen()" :step="25" />
		</div>
		...
		<Dialog v-model:visible="dialogRegalos" header="Regalos Activos" :style="{ width: '600px' }" maximizable modal
			...
			<div class="gifts-container">
				<div v-for="gift in giftListGeneral" :key="gift.id" class="gift-box"
					:class="{ active: isActive(gift.id) }" @click="toggleGift(gift.id)">
					<img :src="gift.image" :alt="gift.name" :class="{ grayscale: !isActive(gift.id) }" />
					<p>{{ gift.points }}</p>
				</div>
			</div>
		</Dialog>
...
export default {
	...
	methods: {
		async startGame() {
			...
			class Duck extends Phaser.GameObjects.Container {
				constructor(scene, x, y, nombre, skin, id) {
					...
					labelBackground
						...
						.fillRoundedRect(-this.labelWidth / 2, -48, this.labelWidth, 20, 10);
			...
			class GameScene extends Phaser.Scene {
				constructor() {
					...
					// Se utiliza cuando el juego termina
				...
				async create() {
					...
					vueDataInstance.$data.socket.on("gift", (user) => {
						...
						if (puntosExiste == undefined && puntosExiste == null) {
							console.log(`regalo ${user.giftId} , ${user.giftName}  no Existe`)
							console.log(user.giftPictureUrl)
							return;
						}
						puntosExiste = puntosExiste * user.repeatCount

							//console.log(`regalo ${user.giftId} con punto ${puntosExiste}`)
							;
					...
					const startVolume = vueDataInstance.$data.volumen / 100;
					this.backgroundMusic.setVolume(startVolume);
					this.quackMusic.setVolume(startVolume);
					this.winMusic.setVolume(startVolume);
				...
				handleEventoApi(user, puntos) {
					const players = this.players;
					const playerIndex = players.findIndex((player) => player.id === user.userId);
				...
				calcularPuntosPatos(pointMax) {
					this.players.forEach((player) => {
						const x = (100 - (Math.abs(pointMax - player.point) / pointMax) * 100) * 8;
		...
		togglePause() {
			...
			let instanciaPhaser = this.game.scene.getScene("scene-game");
			instanciaPhaser.pause(this.pause);
		},
		changeVolumen() {
			let instanciaPhaser = this.game.scene.getScene("scene-game");
...
.gift-box.active {
	border-color: #42b983;
	/* Cambia el borde cuando está activo */
}
...
.grayscale {
	...
	/* Aplica filtro en blanco y negro */
}
